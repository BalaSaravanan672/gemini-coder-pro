...
export class ContextService {
  public static readonly REFERENCED_FILE_PATTERN =
    /\b(?:[\w.-]+\/)+[\w.-]+\.(?:ts|tsx|js|jsx|json|md|txt)\b/g;
  ...
  public static async loadGeminiInstructions(workspaceRoot: string): Promise<string> {
    try {
  ...
  public static shouldInjectContextMap(userText: string): boolean {
    ...
    return /\b(code|file|folder|directory|repo|repository|workspace|project|architecture|architectural|overview|design|structure|system|how it works|how does it work|bug|error|fix|build|test|lint|compile|refactor|implement|edit|patch|command|function|class|ts|tsx|js|json|md)\b/i.test(
      normalized
    );
  }
  ...
  public static extractAtOperatorPaths(userText: string): string[] {
    const pattern = /@([\w.-/]+)/g;
    const matches = [];
    let match;
  ...
  public static async loadAtOperatorContext(
    workspaceRoot: string,
    userText: string
  ): Promise<string> {
    const atPaths = this.extractAtOperatorPaths(userText);
    if (atPaths.length === 0) return '';

    const entries: string[] = [];
    for (const atPath of atPaths) {
      const absolutePath = path.isAbsolute(atPath) ? atPath : path.resolve(workspaceRoot, atPath);
      try {
        const content = await fs.readFile(absolutePath, 'utf8');
        entries.push(`FILE: ${atPath}\n${content}`);
      } catch (error: unknown) {
        const message = error instanceof Error ? error.message : String(error);
        entries.push(`FILE: ${atPath}\n[Unable to read file: ${message}]`);
      }
    }
    return entries.length > 0 ? `EXPLICIT_FILE_CONTEXT:\n${entries.join('\n\n')}` : '';
  }

  public static async loadReferencedFileContext(
    workspaceRoot: string,
    userText: string
  ): Promise<string> {
    ...
    for (const referencedPath of referencedPaths.slice(0, 3)) {
      ...
      try {
        const content = await fs.readFile(absolutePath, 'utf8');
        const excerpt =
          content.length > this.MAX_FILE_CONTEXT_CHARS
  ...
  public static async buildAutoDiscoveryContext(
    workspaceRoot: string,
    userText: string
  ): Promise<string> {
    ...
    try {
      ...
      const bootstrapFiles = Array.from(
        new Set(
        ...
        .filter(
          (file) =>
            !file.includes('node_modules') && !file.includes('dist') && !file.includes('.git')
        )
        .sort((a, b) => {
      ...
      const message = error instanceof Error ? error.message : String(error);
      return `AUTO_DISCOVERY_ERROR: ${message}`;
    }
  }
}
